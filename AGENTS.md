...
Configuration is split across three YAML files: `config/sensors.yaml` (mount positions and directions for 6 TOF sensors plus sensor-to-device mapping), `config/topics.yaml` (deprecated migration note; ROS topic and serial settings are loaded from root `config/config.yaml` via ROS params), `config/map_and_solver.yaml` (world geometry — walls, corners, wall pairs, scene profiles, solver tuning parameters).

In the root Docker Compose stack, `src/positioning/laser_localization/config/` is mounted read-only into the container. The package-local `config/sensors.yaml` and `config/map_and_solver.yaml` are the runtime source for laser sensor geometry, sensor mapping, map geometry, and solver tuning. Root `config/laser/` files are legacy/reference copies only unless Compose is changed to mount them again.
